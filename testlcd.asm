...
; vector address for ext0 and ext1
org 0003H 
    ljmp ISR_EXT0
...
L0:
    nop
    djnz R0, L0 ; 1+4 cycles->5*60.24ns*133=40us // decrement and jump loop back (to L0) if Ro is not equal to zero
    pop AR0
    ret
...
; now that we've finished the required tasks and printing out my name, I can start polling for my switch 
; how about we try interrupts let's enable global interrupts and use pins 5 and 6 as hardware interrupts from our switch
...
;---------------------ISR HANDLER----------------------;
ISR_EXT0:
;push data pointer high and low and accumulator and program counter to the stack
    push ACC
    push psw
    push dpl
    push dph
